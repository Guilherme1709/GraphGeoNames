...
cqlCreate0 = """LOAD CSV WITH HEADERS FROM 'file:///OtherNames_0.csv' AS linha
                ...
                CREATE (Br:PLACE {elevation:linha.elevation, featurecode:linha.featurecode,
                geonameid:linha.geonameid, timezone:linha.timezone, latitude:linha.latitude, 
                admin2_code:linha.admin2_code, population:linha.population,    
                featureclass:linha.featureclass, cc2:linha.cc2,
                admin3_code:linha.admin3_code, name:linha.name, admin1_code:linha.admin1_code,  
                countrycode:linha.countrycode, admin4_code:linha.admin4_code, 
...
cqlCreate1 = """LOAD CSV WITH HEADERS FROM 'file:///OtherNames_0.csv' AS linha
                ...
                MATCH(Br:PLACE{name:linha.name})
...
cqlCreate607 = """LOAD CSV WITH HEADERS FROM 'file:///OtherNames_0.csv' AS linha
                MATCH(location:PLACE), (country:BRAZIL)
...
cqlCreate619 = """LOAD CSV WITH HEADERS FROM 'file:///OtherNames_12.csv' AS linha
                MERGE(location:PLACE), (country:BRAZIL)
                WHERE location.name = linha.name AND country.name = "Brasil"
...
cqlCreate068 = """create index name_index for (br:PLACE) on (br.name);"""
...
#Function to create a relationship between the PLACE of the OtherNames_0 file and the PLACE node.
def check_PLACE_0():
